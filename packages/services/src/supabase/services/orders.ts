...
async function generateSequentialNumber(table: string, column: string, prefix: string): Promise<string> {
...
export async function createOrderWithItems(input: CreateOrderInput): Promise<{ order: OrderRecord; items: OrderItemRecord[]; }> {
  const supabase = getSupabaseServiceClient();

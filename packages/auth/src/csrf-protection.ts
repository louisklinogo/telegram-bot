...
export const security = {
  ...
  generateAuthorizationCode: (): string => {
    return `faw_auth_${crypto.randomBytes(32).toString('base64url')}`;
  },
  
  // Generate secure access token
  generateAccessToken: (): string => {
    return `faw_access_token_${crypto.randomBytes(40).toString('base64url')}`;
  },
  
  // Generate secure refresh token
  generateRefreshToken: (): string => {
    return `faw_refresh_${crypto.randomBytes(40).toString('base64url')}`;
  },
  ...
  generateRateLimitKey: (endpoint: string, identifier: string): string => {
    return `oauth_rl:${endpoint}:${identifier}`;
  },

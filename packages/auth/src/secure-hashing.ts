...
export class SecureHasher {
  ...
  constructor(options: Partial<HashingOptions> = {}) {
    ...
    if (process.env.NODE_ENV === 'production' && this.options.pepper === DEFAULT_OPTIONS.pepper) {
      console.warn('[SECURITY WARNING] Using default pepper in production. Set FAWORRA_HASH_PEPPER environment variable.');
    }
  }
   ...
   */
  generateSecureToken(prefix: string, length: number = 40): string {
...
export class SecurityValidator {
   ...
   */
  static validatePKCE(codeVerifier: string, codeChallenge: string, method: string = 'S256'): boolean {
...
export const verifyApiKey = (apiKey: string, hash: string) => secureHasher.verifyApiKey(apiKey, hash);
export const hashClientSecret = (secret: string) => secureHasher.hashClientSecret(secret);
export const verifyClientSecret = (secret: string, hash: string) => secureHasher.verifyClientSecret(secret, hash);

...
const envSchema = z.object({
  ...
  SUPABASE_ANON_KEY: z.string().min(30, {
...
export function getRedactedEnvInfo(): Record<string, string> {
  ...
  return {
    ...
    SUPABASE_ANON_KEY: validatedEnv.SUPABASE_ANON_KEY ? redactSecret(validatedEnv.SUPABASE_ANON_KEY, 4, 4) : 'not-set',
    ...
    NEXT_PUBLIC_SUPABASE_ANON_KEY: validatedEnv.NEXT_PUBLIC_SUPABASE_ANON_KEY
      ...
      : 'not-set',
    SUPABASE_DB_URL: validatedEnv.SUPABASE_DB_URL ? redactSecret(validatedEnv.SUPABASE_DB_URL, 8, 4) : 'not-set',
    NOTION_API_KEY: validatedEnv.NOTION_API_KEY ? redactSecret(validatedEnv.NOTION_API_KEY, 4, 4) : 'not-set',
    ...
    DATABASE_IDS_LOADED: ['NOTION_CLIENTS_DB_ID','NOTION_INVOICES_DB_ID','NOTION_ORDERS_DB_ID','NOTION_MEASUREMENTS_DB_ID']
      .filter(key => Boolean((validatedEnv as Record<string, unknown>)[key]))
      .length.toString(),
  };
}
...
# (Optional) Notion Integration - used only for migration scripts
...
# Notion Database IDs (optional - only if migration scripts need direct API access)

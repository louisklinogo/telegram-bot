...
import { requestId, type RequestIdOptions } from "hono/request-id";
...
const defaultGenerator = (): string => {
  return `faw_req_${crypto.randomUUID().replace(/-/g, '').substring(0, 16)}`;
};
    ...
    await requestIdMiddleware(c, async () => {
      ...
      try {
        ...
        if (logRequests) {
          ...
          console.log('[Request Complete]', JSON.stringify({
            requestId: reqId,
            traceId,
            method: c.req.method,
            path: c.req.path,

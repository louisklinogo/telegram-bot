...
export const invoices = pgTable(
  ...
  (table) => [
    ...
    index("idx_invoices_unpaid")
      ...
      .where(sql`(status = ANY (ARRAY['pending'::invoice_status, 'overdue'::invoice_status]))`),
...
export const transactions = pgTable(
  ...
  (table) => [
    ...
    index("idx_transactions_team_deleted_date").using(
      ...
      table.transactionDate.desc().nullsFirst().op("uuid_ops")
    ),
    ...
    index("idx_transactions_team_pagination")
      .using(
        ...
        table.id.desc().nullsFirst().op("date_ops")
      )
      .where(sql`(deleted_at IS NULL)`),

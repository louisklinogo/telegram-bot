...
import { teams, clients, orders, measurements, communicationAccounts, communicationThreads, users, instagramContacts, whatsappContacts, communicationMessages, invoices, transactionAllocations, transactions, invoiceItems, orderItems, messageAttachments, bankStatements, communicationOutbox, messageDelivery, usersOnTeam, userInvites, activities, notificationSettings, bankStatementLines, bankStatementAllocations, appointments, communicationTemplates, documents, tags, transactionTags, transactionAttachments, transactionCategories, financialAccounts, transactionEmbeddings, transactionEnrichments } from "./schema";
...
export const transactionEmbeddingsRelations = relations(transactionEmbeddings, ({one}) => ({
	team: one(teams, {
...
export const transactionEnrichmentsRelations = relations(transactionEnrichments, ({one}) => ({
	...
	transaction: one(transactions, {
		fields: [transactionEnrichments.transactionId],
		references: [transactions.id]
	}),
}));
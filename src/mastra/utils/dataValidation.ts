...
export const BaseSchemas = {
  ...
  ghanaPhoneNumber: z.string()
    .regex(/^\+233[0-9]{9}$|^0[0-9]{9}$/, 'Must be a valid Ghana phone number (+233xxxxxxxxx or 0xxxxxxxxx)')
  ...
  notionPageId: z.string()
    .regex(/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/, 'Invalid Notion page ID format'),
...
export const ClientSchemas = {
  create: z.object({
    ...
    referral_source: z.enum(['Instagram', 'WhatsApp', 'Word-of-Mouth', 'Church', 'Walk-in', 'Referral']).optional(),
...
export const InvoiceSchemas = {
  create: z.object({
    ...
    payment_method: z.enum(['Credit Card', 'Bank Transfer', 'PayPal', 'Check', 'Cash', 'Mobile Money', 'Other']).optional(),
...
export const FinanceSchemas = {
  create: z.object({
    ...
    amount: BaseSchemas.currency,
    date: BaseSchemas.date,
    payment_method: z.enum(['Cash', 'Mobile Money (Momo)', 'Bank Transfer', 'Debit Card', 'Credit Card', 'Check']),
  ...
  update: z.object({
    ...
    payment_method: z.enum(['Cash', 'Mobile Money (Momo)', 'Bank Transfer', 'Debit Card', 'Credit Card', 'Check']).optional(),
    linked_order: BaseSchemas.notionPageId.optional(),
    notes: z.string().max(500, 'Notes must be less than 500 characters').optional(),
    receipt_file: BaseSchemas.url,
  }),
...
export const MeasurementSchemas = {
  create: z.object({
    ...
    chest: z.number().min(8, 'Chest must be at least 8 inches').max(65, 'Chest must be less than 65 inches').optional(),
    shoulder: z.number().min(8, 'Shoulder must be at least 8 inches').max(65, 'Shoulder must be less than 65 inches').optional(),
    sleeve: z.number().min(8, 'Sleeve must be at least 8 inches').max(65, 'Sleeve must be less than 65 inches').optional(),
    waist: z.number().min(8, 'Waist must be at least 8 inches').max(65, 'Waist must be less than 65 inches').optional(),
    hip: z.number().min(8, 'Hip must be at least 8 inches').max(65, 'Hip must be less than 65 inches').optional(),
    lap: z.number().min(8, 'Lap must be at least 8 inches').max(65, 'Lap must be less than 65 inches').optional(),
    calf: z.number().min(8, 'Calf must be at least 8 inches').max(65, 'Calf must be less than 65 inches').optional(),
    neck: z.number().min(8, 'Neck must be at least 8 inches').max(65, 'Neck must be less than 65 inches').optional(),
    stomach: z.number().min(8, 'Stomach must be at least 8 inches').max(65, 'Stomach must be less than 65 inches').optional(),
    bicep_round: z.number().min(8, 'Bicep round must be at least 8 inches').max(65, 'Bicep round must be less than 65 inches').optional(),
    top_length: z.string().regex(/^\d+(\.\d+)?(\/\d+(\.\d+)?)?$/, 'Top length must be a number or dual entry like "31/37"').optional(),
    notes: z.string().max(500, 'Notes must be less than 500 characters').optional(),
  }),
  ...
  update: z.object({
    ...
    top_length: z.string().regex(/^\d+(\.\d+)?(\/\d+(\.\d+)?)?$/).optional(),
...
export const ToolOperationSchemas = {
  ...
  recordUpdate: z.object({
    ...
    message: 'record_id is required for single updates, record_ids is required for bulk updates',
    path: ['record_id'],
  }),
  
  businessAnalytics: z.object({
...
export class ValidationHelper {
   ...
   */
  static validateDateRange(start: string, end: string): {
    valid: boolean;
    error?: string;
  } {
   ...
   */
  static validateInvoiceAmounts(amount: number, amountPaid: number): {
    valid: boolean;
    amountDue?: number;
    error?: string;
  } {

...
import { rateLimitInfoSchema, securityEventSchema, type SecurityEvent } from '../telegram/schemas';
import { isProduction } from '../config/validateEnv';
...
async function logSecurityEvent(event: SecurityEvent): Promise<void> {
  try {
    ...
    if (isProduction()) {
      ...
      console.warn('⚠️  Security Event:', validatedEvent.event_type, 
 ...
 */
function getClientIdentifier(c: Context): { userId?: number; ip: string; userAgent?: string } {
  ...
  return async (c: Context, next: Next) => {
    ...
    try {
      ...
      if (userId) {
        ...
        c.header('X-RateLimit-Limit', finalConfig.userLimit.toString());
        c.header('X-RateLimit-Remaining', Math.max(0, finalConfig.userLimit - userCount).toString());
        c.header('X-RateLimit-Reset', Math.floor((Date.now() + finalConfig.userWindow * 1000) / 1000).toString());
      }
      ...
      c.header('X-Global-RateLimit-Limit', finalConfig.globalLimit.toString());
      c.header('X-Global-RateLimit-Remaining', Math.max(0, finalConfig.globalLimit - globalCount).toString());
 ...
 */
export async function getRateLimitStatus(userId?: number, ip?: string): Promise<{

...
export function OrderSheet({ open, onOpenChange, order }: OrderSheetProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    deposit_amount: order
      ...
      : 0,
    due_date: order
      ? ((order as any).due_date || (order as any).dueDate || "").split("T")[0] || ""
      : "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEdit) {
      await (updateMutation as any).mutateAsync({ id: order.id, ...orderData });
    } else {
        ...
        <SheetHeader>
          ...
          <SheetDescription>
            {isEdit
              ...
              : "Add client, items, and payment information."}
          </SheetDescription>
        </SheetHeader>

...
export function ClientsView({ initialClients = [] }: ClientsViewProps) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, refetch } = useSuspenseInfiniteQuery({
    ...
    initialData: initialClients.length > 0
      ? {
  ...
  const filteredClients = useMemo(() => {
    ...
    if (activeFilters.referralSource) {
      result = result.filter((c: any) => 
        c.referralSource?.toLowerCase() === activeFilters.referralSource?.toLowerCase()
      );
    }
    ...
    if (activeFilters.minRevenue) {
      result = result.filter((c: any) => 
        Number(c.totalRevenue || 0) >= activeFilters.minRevenue!
      );
    }
    ...
    if (activeFilters.maxRevenue) {
      result = result.filter((c: any) => 
        Number(c.totalRevenue || 0) <= activeFilters.maxRevenue!
      );
    }
  ...
  const handleBulkCopy = () => {
    ...
    toast.success(`Copied ${selectedIds.size} client${selectedIds.size > 1 ? 's' : ''} to clipboard`);
  };
  ...
  const handleCSVUpload = async (data: any[]) => {
    ...
    if (errorCount > 0) {
      toast.error(`Failed to import ${errorCount} client${errorCount > 1 ? 's' : ''}. Check console for details.`);
  ...
  return (
    <>
      <div className="flex flex-col gap-6">
        ...
        {filteredClients.length === 0 && (search || tagFilter || Object.keys(activeFilters).length > 0) && (
          <EmptyState
            title="No results"
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {isColumnVisible("name") && <TableHead className="sticky left-[50px] bg-background z-20">Name</TableHead>}
                      ...
                      <TableCell>
                        {client.totalRevenue && Number(client.totalRevenue) > 0 ? (
                          <span className="font-medium">
                            GHS {Number(client.totalRevenue).toFixed(2)}
                          </span>
                        ) : (

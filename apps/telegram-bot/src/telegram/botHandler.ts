...
import { type FileHandlerResult, processDocument, processImage } from "../mastra/tools/fileHandler";
...
Just send me your details and I'll handle the rest! 🚀
  ...
  bot.on("message:photo", async (ctx) => {
    ...
    try {
      ...
      const result: FileHandlerResult = await processImage(chatId, largestPhoto.file_id, caption);

      if (!result.success) {
        console.error("Image processing failed:", result.error);
        await ctx.reply(
          "❌ Sorry, I had trouble processing your image. Please try again with a different image.",
        );
        return;
      }
      ...
      if (userId) {
        try {
          ...
          if (
            response.success &&
            response.message &&
            response.message !== "✅ **File Processed**"
          ) {
  ...
  bot.on("message:document", async (ctx) => {
    ...
    console.log(
      `📄 Processing document from chat ${chatId}, file_id: ${document.file_id}, name: ${document.file_name}`,
    );

    try {
      await ctx.replyWithChatAction("upload_document");

      const result: FileHandlerResult = await processDocument(
        chatId,
        document.file_id,
        document.file_name,
        caption,
      ...
      if (!result.success) {
        ...
        await ctx.reply(
          "❌ Sorry, I had trouble processing your document. Please try again with a different file.",
        );
        return;
      }
    ...
    try {
      ...
      if (isMeasurementData) {
        const processingTimeout = setTimeout(async () => {
          try {
            const message =
              text.length > 100 && /[A-Z]{2}\s+\d+/.test(text)
        ...
        const originalProcessMessage = apiClient.processMessage(
          text,
          userId,
          chatId,
          username,
      ...
      if ((response as { success?: boolean }).success) {
        ...
        if (response.metadata?.steps) {
          console.log(
            `Processed message for user ${userId}: ${response.metadata.steps} steps, ${response.metadata.usage?.totalTokens || "unknown"} tokens`,
          );
        }
      } else {
        try {
      ...
      if (error instanceof Error && error.message === "Processing timeout") {
        try {
          ...
          await ctx.reply(
            "😅 Your request took longer than expected to process. For complex operations like invoices or measurements, please wait as workflows may still be running in the background. You'll receive a notification once complete!",
          );
        } catch (replyError) {

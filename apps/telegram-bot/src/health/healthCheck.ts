    ...
    try {
      ...
      for (let attempt = 0; attempt <= this.retries; attempt++) {
        try {
          ...
          const result = await Promise.race([this.checkHealth(), timeoutPromise]);
...
/**
 * Check Supabase connectivity via lightweight query
 */
export class SupabaseHealthChecker extends BaseHealthChecker {
  private getClient = getSupabaseServiceClient;

  constructor(logger?: any, tracer?: any) {
    super(4000, 1, logger, tracer);
  }
  ...
  async checkHealth(): Promise<HealthCheckResult> {
    try {
      ...
      const { data, error } = await client.from("mastra_kv").select("key").limit(1);
...
export class HealthCheckManager {
  ...
  initializeCheckers() {
    ...
    if (env.INVOICE_GENERATOR_API_KEY) {
      this.checkers.push(
        new InvoiceGeneratorHealthChecker(env.INVOICE_GENERATOR_API_KEY, this.logger, this.tracer),
      );
      this.logInfo("Added Invoice Generator health checker");
    }
    // Add Supabase checker when configured
    if (env.SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY) {
      this.checkers.push(new SupabaseHealthChecker(this.logger, this.tracer));
      this.logInfo("Added Supabase health checker");
    }

    // Add Telegram checker if bot token is available
    if (env.TELEGRAM_BOT_TOKEN) {
      this.checkers.push(
        new TelegramHealthChecker(env.TELEGRAM_BOT_TOKEN, this.logger, this.tracer),
      );
  ...
  addMastraCheckers(mastraInstance: any) {
    ...
    Object.keys(agents).forEach((agentName) => {
      this.checkers.push(
        new AgentHealthChecker(mastraInstance, agentName, this.logger, this.tracer),
      );
    ...
    Object.keys(workflows).forEach((workflowName) => {
      this.checkers.push(
        new WorkflowHealthChecker(mastraInstance, workflowName, this.logger, this.tracer),
      );

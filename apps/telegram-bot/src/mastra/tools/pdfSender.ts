...
export const pdfSender = createTool({
  ...
  inputSchema: z.object({
    chat_id: z
      .number()
      .optional()
      .describe("The chat ID to send the PDF to (will use runtime context if not provided)"),
  ...
  execute: async ({ context, runtimeContext }, _options) => {
    ...
    try {
      let document: InputFile | string | undefined;
      let filename: string = "";
      ...
      if (pdf_url) {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            if (urlTest.ok) {
              ...
              console.warn(
                `⚠️ Cloudinary URL not accessible (attempt ${attempt}/3), status:`,
                urlTest.status,
              );
              ...
              if (urlTest.status === 401 && attempt < 3) {
                ...
                try {
                  ...
                  if (getTest.ok) {
                    console.log(
                      `✅ Cloudinary URL accessible via GET request (attempt ${attempt})`,
                    );
                    urlTestSuccessful = true;
                    break;
                  }
            ...
            lastError = error;
            console.warn(
              `❌ URL test failed (attempt ${attempt}/3):`,
        ...
        if (urlTestSuccessful) {
          ...
          if (pdf_path) {
            ...
            throw new Error(
              `Cloudinary URL inaccessible after 3 attempts and no local file provided: ${lastError instanceof Error ? lastError.message : "Unknown error"}`,
            );
          }
        }
      }
      ...
      if (pdf_path && !document) {
        ...
        if (fileSizeInMB > 50) {
          throw new Error(
            `PDF file is too large (${fileSizeInMB.toFixed(2)}MB). Maximum size is 50MB.`,
          );
        }
...
export const sendInvoicePdf = async (
  ...
  const { pdf_path, pdf_url, customerName, invoiceNumber, totalAmount, reply_to_message_id } =
    options;

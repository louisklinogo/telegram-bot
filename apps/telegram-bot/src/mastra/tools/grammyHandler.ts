...
type SendMessageOptions = Parameters<BotInstance["api"]["sendMessage"]>[2];
...
export const setBotInstance = <T extends Context>(bot: Bot<T>) => {
  ...
  botInstance = bot as unknown as BotInstance;
};
...
export const grammyHandler = createTool({
  ...
  execute: async ({ context }, _options) => {
    ...
    try {
      // Use minimal casting to satisfy grammy types while allowing reply_markup
      ...
      if (keyboard) {
        messageOptions.reply_markup = keyboard as unknown;
      }

      const message = await botInstance?.api.sendMessage(
        chat_id,
        text,
        messageOptions as SendMessageOptions,
      );
...
export const sendTextMessage = async (
  ...
  try {
    ...
    type ToolExecute = NonNullable<typeof grammyHandler.execute>;
    const tool = grammyHandler as typeof grammyHandler & {
      execute: ToolExecute;
    };

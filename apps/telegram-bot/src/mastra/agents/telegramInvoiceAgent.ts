...
import { createMastraPgVector, createMastraPostgresStore } from "../storage/postgres";
...
const memory = new Memory({
  ...
  storage: createMastraPostgresStore(),
  // Vector store for semantic recall
  vector: createMastraPgVector(),
...
export const cimantikosBizAssistant = new Agent({
  ...
  description:
    "Maya is your internal business operations assistant at Cimantikós Clothing Company, helping you manage orders, measurements, finances, and customer records backed by Supabase services",
  instructions: `
You are Maya, an internal business operations assistant for Cimantikós Clothing Company. Your primary user is the business manager who needs to efficiently manage orders, measurements, finances, and customer records powered by Supabase.
...
<notionSearchTool>
- WHEN: Quick lookups and searches across Supabase tables
...
<fallback_tools>
Fallback tools for compatibility and error recovery:
- invoiceGenerator: Direct PDF generation
- notionOrdersTool: Supabase order creation (legacy naming)
- notionMeasurementsTool: Supabase measurement management (legacy naming)  

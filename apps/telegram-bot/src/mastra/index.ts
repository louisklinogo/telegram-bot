...
import { createHealthResponse, HealthCheckManager } from "../health/healthCheck";
import { cimantikosBizAssistant } from "./agents/telegramInvoiceAgent";
import { mastraPostgresStore } from "./storage/postgres";
...
export const mastra = new Mastra({
  ...
  // Agent uses PgVector with Google embeddings for semantic recall
      ...
      registerApiRoute("/health/metrics", {
        ...
        handler: async (c) => {
          ...
          const metrics = {
            ...
            services: health.services.reduce(
              (acc, service) => {
                acc[service.name] = {
